...
AVAILABLE_TEXT_MODELS = {
    "google_gemini_2_0_flash": {
        ...
        "limit": DEFAULT_FREE_REQUESTS_DAILY,
...
if not GOOGLE_GEMINI_API_KEY or "YOUR_GOOGLE_GEMINI_API_KEY" in GOOGLE_GEMINI_API_KEY or "AIzaSy" not in GOOGLE_GEMINI_API_KEY:
    logger.warning("Google Gemini API key is not set correctly or uses a placeholder.")
else:
    try:
...
if not CUSTOM_GEMINI_PRO_API_KEY or "YOUR_CUSTOM_KEY" in CUSTOM_GEMINI_PRO_API_KEY or "sk-" not in CUSTOM_GEMINI_PRO_API_KEY:
    logger.warning("Custom Gemini Pro API key is not set correctly or uses a placeholder.")
...
def get_user_actual_limit_for_model(user_id: int, model_key: str, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if limit_type == "daily_free":
        ...
        if user_subscription.get('level') in ["pro", "advanced", "custom_api"]:
        ...
        elif user_subscription.get('level') == "advanced":
             actual_limit = model_config.get("subscription_daily_limit_advanced", model_config.get("subscription_daily_limit_pro", 0))
        else:
            actual_limit = model_config.get("limit_if_no_subscription", 0)
    elif limit_type == "subscription_daily_premium":
        if user_subscription.get('level') == "advanced":
             actual_limit = model_config.get("subscription_daily_limit", 0)
        else:
            actual_limit = model_config.get("limit_if_no_subscription", 0)
    elif limit_type == "subscription_daily_custom":
        if user_subscription.get('level') == "custom_api" or user_subscription.get('level') == "advanced":
            actual_limit = model_config.get("subscription_daily_limit", 0)
        else:
            actual_limit = model_config.get("limit_if_no_subscription", 0)
    else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.setdefault('subscription_info', {'level': None, 'valid_until': None})
...
async def usage_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    usage_text += "\n"
    usage_text += escape_markdown("Для изменения подписки или при возникновении вопросов обратитесь к администратору.", version=2)
    try:
    ...
    if api_type == "google_genai":
        ...
        if not actual_api_key or "sk-" not in actual_api_key:
            ...
            payload = {
                ...
                "top_p": 1,
                "temperature": 0.75
                # "response_format": {"type": "text"}  # Убрано, так как документация допускает строку или объект, но может быть необязательным
            }
            headers = {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
            ...
            try:
                ...
                try:
                    error_content = e_http.response.json()
                except json.JSONDecodeError:
                    error_content = e_http.response.text
                if e_http.response.status_code == 422:
                    reply_text = f"Ошибка 422: Неверный запрос к Custom API. Детали: {error_content}"
                else:
                    reply_text = f"Ошибка сети ({e_http.response.status_code}) при обращении к '{selected_model_details['name']}'. Детали: {error_content}"
