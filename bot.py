...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        error_message_text = f"К сожалению, произошла ошибка при обработке вашего запроса с моделью *{escaped_display_name}*\\. Пожалуйста, попробуйте позже или смените модель/режим\\."
        try:
            await update.message.reply_text(error_message_text, parse_mode=ParseMode.MARKDOWN_V2)
        except: # Если даже сообщение об ошибке не отправляется с Markdown
             await update.message.reply_text(f"К сожалению, произошла ошибка при обработке вашего запроса. Пожалуйста, попробуйте позже.")
