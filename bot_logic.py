...
async def web_app_data_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "set_agent" or action == "set_model":
        ...
        # Получаем сообщение, генерируем ответ и КЛАДЕМ ЕГО В "ПОЧТОВЫЙ ЯЩИК"
        payload = data.get("payload", {})
        # ... (вся логика получения текста, проверки лимитов, вызова ИИ - как в прошлый раз)
        # ... (этот блок кода можно скопировать из вашего текущего handlers.py)
        ...
        await context.bot.send_message(chat_id=user_id, text=user_message_text)
        ...
        if not can_proceed:
            error_for_app = [{"sender": "bot", "text": limit_message, "timestamp": datetime.utcnow().isoformat()}]
            await firestore_service._db.collection(BotConstants.FS_APP_MESSAGES_COLLECTION).document(str(user_id)).set({"messages": error_for_app})
            return

        ai_response_text = "..." # Здесь вызов вашего ai_service
        # ...
        # В конце, после получения ответа ai_response_text:
        final_reply, _ = smart_truncate(ai_response_text, CONFIG.MAX_MESSAGE_LENGTH_TELEGRAM)
        
        # Отправляем ответ в основной чат
        await context.bot.send_message(chat_id=user_id, text=final_reply)

        # И сохраняем его для Mini App
        bot_message_for_app = [{"sender": "bot", "text": final_reply, "timestamp": datetime.utcnow().isoformat()}]
